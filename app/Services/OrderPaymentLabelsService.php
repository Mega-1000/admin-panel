...
readonly class OrderPaymentLabelsService
{
    ...
    public function calculateLabels(Order $order, ?bool $calculateRelated = null): void
    {
        ...
        dd(   round($sumOfGrossValues) + round($depositPaidData['returnedValue']) - round($depositPaidData['offerFinanceBalance']) - round($depositPaidData['wtonValue']), round($sumOfGrossValues), round($depositPaidData['returnedValue']), round($depositPaidData['offerFinanceBalance']), round($depositPaidData['wtonValue']));
        if (
