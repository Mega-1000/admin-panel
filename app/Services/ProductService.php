...
class ProductService
{
     ...
     * @param null $deliveryAddressLatLon
     * @return array
     */
    public function getVariationsFromProducts($products, $deliveryAddressLatLon = null): array
    {
        $productsVariation = [];
        foreach ($products as $product) {
            ...
            $productVar = $this->productRepository->findByField('product_group', $product->product_group);
            foreach ($productVar as $prod) {
                $firm = $this->firmRepository->findByField('symbol', $prod->product_name_supplier);
                $radius = 0;
                ...
                $productsVariation[$product->id][] = $array;

            foreach ($productsVariation as $variation) {
                if (isset($productsVariation[$product->id])) {
                    $productsVariation[$product->id] = collect($variation)->sortBy('different', 1, true);
                }
            }
        }
