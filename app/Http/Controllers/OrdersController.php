...
class OrdersController extends Controller
{
    ...
    public function sumOfOrders($order)
    {
	    ...
	    if($order->bookedPaymentsSum() > 0 && $order->bookedPaymentsSum() < 2) {
        ...
        if($sum - $sumOfPayments < 2 || $sum - $sumOfPayments > -2) {
            ...
            $mainOrderSum = $sum - $sumOfPackages;
        }
		...
		if($connectedOrder->bookedPaymentsSum() > 0 && $connectedOrder->bookedPaymentsSum() < 2) {
            ...
            if($sum - $sumOfPayments < 2 && $sum - $sumOfPayments > -2) {
                ...
                $connOrderSum = $sum - $sumOfPackages;
            }
