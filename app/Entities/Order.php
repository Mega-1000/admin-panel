...
class Order extends Model implements Transformable
{
    ...
    public function getItemsGrossValueForUs(): float
    {
        ...
        foreach ($this->items as $item) {
            $totalOfProductsPrices += ((float)$item->net_purchase_price_commercial_unit_after_discounts * intval($item->quantity) * 1.23);
        }
...
//        ]);
    ...
    public function sellingInvoiceValue(): HasMany
    {
        return $this->hasMany(OrderInvoiceValue::class)->where('type', 'SELLING');
    }
}
