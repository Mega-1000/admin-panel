...
class GenerateAdvancedXmlForNexoJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        foreach ($files as $file) {
            Storage::disk('xmlAdvanccedDisk')->delete($file);
        }
        ...
        foreach ($orders as $order) {
            try {
                ...
                Storage::disk('xmlAdvanccedDisk')->put($xmlFileName, mb_convert_encoding($xml, "UTF-8", "auto"));
                $preventionArray = [];

                $fileNames[] = $xmlFileName;

                AddLabelService::addLabels($order, [121], $preventionArray, [], Auth::user()?->id);
            } catch (Throwable $ex) {
        ...
        if (count($fileNames) > 0) {
            ...
            foreach ($fileNames as $fileName) {
                Storage::disk('xmlAdvanccedDisk')->delete($fileName);
            }
