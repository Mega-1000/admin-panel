...
class ValidateSubiekt implements ShouldQueue
{
    ...
    public function handle(OrderRepository $orderRepository)
    {
        ...
        foreach($toCheck as $order){
            $invoiceRow = DB::table('gt_invoices')->where('order_id', $order->order_id)->where('gt_invoice_status_id', '19')->first();
            ...
            $noData = DB::table('gt_invoices')->where('order_id', $order->order_id)->where('gt_invoice_status_id', '13')->first();
            if(!empty($invoiceRow)) {
                \Mailer::create()
                ...
                \Mailer::create()
                    ->to($order->customer->login)
                    ->send(new DifferentCustomerData('Wybór danych do wystawienia faktury', $order->order_id, $noData->id));
            }
        }
    }
}
