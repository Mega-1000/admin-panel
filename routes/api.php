...
Route::get('styro-warehouses', function () {
    ...
    function geocodeAddress($apiKey, $address) {
        // URL endpoint for the Google Maps Geocoding API
        $url = "https://maps.googleapis.com/maps/api/geocode/json";

        // Encode the address for the URL request
        $encodedAddress = urlencode($address);

        // Build the full URL with the encoded address and API key
        $fullUrl = $url . "?address=" . $encodedAddress . "&key=" . $apiKey;
        ...
        if ($response_data['status'] == 'OK') {
            ...
            return "Geocoding failed with status: " . $response_data['status'];
        }
    }


    $firms = \App\Entities\Firm::whereHas('products', function ($query) {$query->where('variation_group', 'styropiany');})->with('warehouses')->get()->pluck('warehouses')->flatten();

    $apiKey = 'AIzaSyCXhsc2iSDv1aBOGwLxpAWFk2VS3aOhZKI';
    foreach ($firms as $firm) {
        $firm->cordinates = geocodeAddress($apiKey, $firm->address->stringify());
        $firm->save();
    }
});
