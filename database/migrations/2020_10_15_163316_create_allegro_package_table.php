...
class CreateAllegroPackageTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('allegro_package', function (Blueprint $table) {
            $table->increments('id');
            $table->dateTime('allegro_operation_date')
                ->comment('Data operacji Allegro pobierana z pliku CSV');
            ...
            $table->decimal('ads_campaign_fee', 9, 2)
                ->comment('Opłata za kampanie ADS');
            $table->decimal('bill_correction', 9, 2)
                ->comment('Korekta rachunku');
            $table->decimal('preference_auction_fee', 9, 2)
                ->comment('Opłata za wyróżnienie aukcji Allegro');
            $table->decimal('booked_payment', 9, 2)
                ->comment('Wpłata zaksiegowana (za faktury ALLEGRO )');
            $table->decimal('month_summary', 9, 2)
                ->comment('Podsumowanie miesiąca ALLEGRO');
