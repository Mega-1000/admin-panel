                    ...
                    @foreach($finalItems as $item)
                        <div class="border-b py-4">
                            @foreach($item as $product)
                                @php
                                    $productPrice = (empty(\App\Entities\ChatAuctionOffer::whereHas('product', function ($q) use ($product) {$q->where('parent_id', $product->parent_id);})
                                                                    ...
                                                                    ->where('chat_auction_id', $order->chat->auctions->first()->id)
                                                                    ->orderBy('basic_price_net', 'asc')
                                                                    ->first()->basic_price_net * 1.23) * $product->packing->numbers_of_basic_commercial_units_in_pack;
                                @endphp
                            {{$product->number_of_sale_units_in_the_pack}}
                                ...
                                Ilość paczek: {{ round($product->quantity, 2) }} <br>
                                Cena brutto paczki w zamówieniu: {{ $productPrice }}
                            </span>
                    ...
                    productGroups.forEach(function(productGroup) {
                        ...
                        if (checkedCheckbox) {
                            ...
                            products.forEach(span => {
                                ...
                                const quantity = parseInt(span.textContent.match(/Ilość paczek: ([\d\.]+)/)[1]);
                                const price = parseFloat(span.textContent.match(/Cena brutto paczki w zamówieniu: ([\d\.]+)/)[1]);
                ...
                function sendOrder() {
                    ...
                    fetch('/submit-order/{{ $order->id }}', {
                        ...
                        .then(async (data) => {
                            ...
                            window.location.href = `https://mega1000.pl/payment?token={{ $order->token }}&total=${totalPrice + 50}&credentials={{$order->customer->login}}:{{$order->customer->phone}}&message=Zapłać teraz 150zł zaliczki a my skontaktujemy cię bezpośrednio z producentem`;
                        })
                        .catch(error => {
