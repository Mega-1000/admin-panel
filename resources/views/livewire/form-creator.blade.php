    ...
    <hr>
        <div wire:sortable="updateElementOrder" wire:sortable-group="form-elements">
            @foreach($this->form?->elements ?? [] as $element)
                <div wire:key="element-{{ $element->id }}" wire:sortable.item="{{ $element->id }}" class="flex">
                    <div class="flex justify-content-between" wire:sortable.handle>
                        <div>
                            <div>
                                typ: {{ $element->type }}
                            </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center">
            ...
            <div class="modal-container bg-white w-96 mx-auto rounded-lg shadow-lg z-50">
                <div class="modal-content py-4 px-6">
                    <form wire:submit.prevent="createNewElement">
                        <h3 class="text-lg font-semibold mb-4">Dodaj nowy element</h3>
                        ...
                        @foreach($this->type ? $this->elementTypes[$this->type]['inputs'] : [] as $input)
                            <input class="form-control mt-4" wire:model="newElement.{{$input['name']}}" placeholder="{{$input['placeholder']}}">
                        @endforeach
