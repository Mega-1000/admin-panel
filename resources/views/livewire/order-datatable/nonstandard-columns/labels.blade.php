...
@if($labelGroupName === 'info dodatkowe')
    ...
    @if($hasLabel95 && $or->getValue() < 3000 && Order::where('id', $or->id)->whereHas('items', function ($query) {$query->whereHas('product', function ($subQuery) {$subQuery->where('variation_group', 'styropiany');});})->first())
        <div style="color: red">
...
@if($labelGroupName === 'transport')
    ...
    @foreach($or->dates()?->first()?->toArray() ?? [] as $k => $date)
        @if(array_key_exists($k, $dateTranslations))
            {{ $dateTranslations[$k] ?? '' }}: {{ isset($date) ? \Carbon\Carbon::parse($date)->timezone('Europe/Warsaw')->format('m-dH:i') : 'Brak' }}
            <br>
        @endif
    @endforeach
