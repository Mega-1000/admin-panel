...
@section('app-header')
    <h1 class="page-title">
        <i class="voyager-person"></i> @lang('column_visibilities.visibilities.edits'){{$visibilities->name}} @lang('column_visibilities.visibilities.edits2')
        {{$roleName}} @lang('column_visibilities.visibilities.edits3') {{$moduleName}}
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    ...
                    <div class="panel-body">
                        ...
                        <form action="{{ action('ColumnVisibilitiesController@visibilitiesUpdate',['module_id'=>$module_id,'role_id'=>$role_id,'id'=>$visibilities->id]) }}"
                              method="POST">
                            {{ csrf_field() }}
                            {{ method_field('put') }}
                            <div class="customer-general" id="general">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="display_name" name="display_name" value="{{$visibilities->display_name}}">
                                </div>
                                ...
                                Zaznacz kolumny które mają być widoczne
                                 @if($isNumberColumns)
                                     @foreach($columns as $key => $row)
                                            ...
                                            @if(isset($visibilities->columnName[$row]))
                                                <input type="checkbox" class="form-check-input" checked name="columnName[{{$row}}]" id="columnName[{{$row}}]">
                                            @else
