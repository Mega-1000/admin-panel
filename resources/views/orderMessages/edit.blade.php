...
@section('app-header')
    <h1 class="page-title">
        ...
        <a style="margin-left: 15px;" href="{{ action('OrdersController@edit', ["id" => $orderMessage->order_id]) }}"
...
@section('table')
    ...
    @endif
    <form action="{{ action('OrdersMessagesController@update', $orderMessage->id) }}" method="POST">
        {{ csrf_field() }}
        {{ method_field('put') }}
        <div class="firms-general" id="orderTask">
            ...
            <div class="form-group">
                <label for="status">@lang('order_messages.form.status')</label>
                <select name="status" id="status" class="form-control">
                    @if($orderMessage->status == 'OPEN')
            ...
            <div class="form-group">
                <label for="type">@lang('order_messages.form.type')</label>
                <select name="type" id="type" class="form-control">
                    @if($orderMessage->type == 'GENERAL')
                    ...
                    @elseif($orderMessage->type == 'SHIPPING')
                        <option value="GENERAL">Ogólne</option>
                        <option value="SHIPPING" selected>Wysyłka</option>
                        <option value="WAREHOUSE">Magazyn</option>
                        <option value="COMPLAINT">Skarga</option>
                    @elseif($orderMessage->type == 'WAREHOUSE')
                        <option value="GENERAL">Ogólne</option>
                        <option value="SHIPPING">Wysyłka</option>
                        <option value="WAREHOUSE" selected>Magazyn</option>
                        <option value="COMPLAINT">Skarga</option>
                    @elseif($orderMessage->type == 'COMPLAINT')
                        <option value="GENERAL">Ogólne</option>
                        <option value="SHIPPING">Wysyłka</option>
                        <option value="WAREHOUSE">Magazyn</option>
                        <option value="COMPLAINT" selected>Skarga</option>
                    @endif
...
@section('scripts')
    <script>
        ...
        breadcrumb.append("<li class='active'><a href='/admin/orders/{{$orderMessage->order_id}}/edit'>Wiadomości</a></li>");
