...
@section('app-header')
    ...
    <button style="height: 36px; margin-bottom: 8px;" type="submit" form="ordersReturn" id="submitOrderAndStay" name="submit"
            value="updateAndStay"
            class="btn btn-primary">Zapisz i pozostań</button>
@endsection
    ...
    <form id="ordersReturn" enctype="multipart/form-data" action="{{ action('OrderReturnController@store', ['id' => $order->id])}}"
        ...
        @foreach($order->items as $item)
            <div class="col-md-6">
                ...
                <div id="form-box{{$loop->iteration}}" class="form-return" @if( count($item->realProductPositions()) && $order->returnPosition($item->realProductPositions()[0]['id'])!==null)style="display: block;" @endif>

                @if(!count($item->realProductPositions()))
                    <div class="row">
                        <div class="form-group" >
                            <label for="value_of_items_gross" class="col-sm-4 control-label">Pozycja na magazynie</label>
                            <div class="col-sm-2">
                ...
                @endif
                    <div class="row">
                        <div class="form-group" >
                            <label for="value_of_items_gross" class="col-sm-4 control-label">Towar nieuszkodzony (max ilość: {{$item->quantity}})</label>
                            <div class="col-sm-2">
                                <input type="number" class="form-control" id="undamaged" name="return[{{$loop->iteration}}][undamaged]" 
                                    @if(
                    ...
                    <div class="row">
                        <div class="form-group" >
                            <label for="value_of_items_gross" class="col-sm-4 control-label">Towar uszkodzony (max ilość: {{$item->quantity}})</label>
                            <div class="col-sm-2">
                                <input type="number" class="form-control" id="damaged" name="return[{{$loop->iteration}}][damaged]"
                                    @if(
                                        count($item->realProductPositions()) && 
                                        $order->returnPosition($item->realProductPositions()[0]['id'])!==null
                                    )
                    ...
                    <div class="row">
                        <div class="form-group" >
                            <label for="value_of_items_gross" class="col-sm-2 control-label">Opis uszkodzenia</label>
                            <div class="col-sm-8">
                                <textarea rows="5" cols="40" class="form-control" id="description" name="return[{{$loop->iteration}}][description]" 
                                    @if(
                                        count($item->realProductPositions()) && 
                                        $order->returnPosition($item->realProductPositions()[0]['id'])!==null
                                    ) 
                                ...
                                >@if(
                                    count($item->realProductPositions()) && 
                                    $order->returnPosition($item->realProductPositions()[0]['id'])!==null
                                ){{$order->returnPosition($item->realProductPositions()[0]['id'])->description}}@endif</textarea>
                            </div>
                        </div>
                    </div>
                    <div class="row">
        ...
        <button type="submit" form="ordersReturn" id="submit" name="submit" value="update" class="btn btn-primary">Zapisz</button>
    </form>
