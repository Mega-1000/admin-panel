                    ...
                    @foreach($orders as $order)
                        <div class="bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                            <div class="p-4">
                                ...
                                @if($order->labels->contains('id', 77))
                                    <div class="bg-red-100 border-l-4 border-yellow-500 text-yellow-700 p-3 mb-4">
                                        <p class="font-bold">
                                            ...
                                            <a
                                                href="https://new.mega1000.pl/magazyn/awizacja/{{$order->orderWarehouseNotifications->first()->id}}/{{ App\Entities\Firm::where('symbol', $order->items->first()->product->manufacturer)->first()->id }}/{{ $order->id }}"
                                                target="__blank"
                                ...
                                @if($order->labels->contains('id', 260))
                                    <div class="bg-green-100 border-l-4 border-yellow-500 text-yellow-700 p-3 mb-4">
                                        ...
                                        <hr>
                                        <a href="{{ $order->paymentConfirmation->file_url }}" target="_blank">Potwierdzenie przelewu zostało wysłane
                                        <br><br>
                                        {{ substr(explode('/', $order->paymentConfirmation->file_url)[4], -15) }}
                                        </a>
                                        <br>
                                        <br>
                            ...
                            @if($order->labels->contains('id', 66))
                                ...
                                <div class="mt-4">
                                    <a href="/firm-panel-actions/order/{{ $order->id }}" class="text-white bg-blue-500 py-2 px-4 rounded font-medium ">Szczegóły zamówienia</a>
                                </div>
                            </div>
                        </div>
                    @endforeach
